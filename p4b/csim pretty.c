 ...
 * param - curSet     this is the current set that was extracted from addr in access_data
 * param - parentNode this is the parent of the node that we will make the head of the list
 */
...
void access_data(mem_addr_t addr)
{
	...
	if (E == 1)
	{
		...
		if (cache[curSet]->valid)
		{
			...
			//else there was a conflict miss
			else
			{
		...
		//else v-bit is 0, i.e. no item in the block; this is a cold miss
		else
		{
			//set tag id and v-bit
			cache[curSet]->tag = tagID;
	...
	//else not a direct-mapped cache, i.e. lines/set > 1
	else
	{
		//set line ptrs that refer to the parent of the current line, along with the parent's parent
		...
		for (parentLine = cache[curSet]; parentLine->next->next != NULL; parentLine = parentLine->next)
		{
